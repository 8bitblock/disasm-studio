...
- The compact shell merges document selection into the header, retains the full feature-page strip and uses one primary execution row with secondary actions in More. Binary View uses flat representation tabs, one function/context row, permanent Functions/Strings lists, a central detail drawer and a register-first right rail. Workspace, Go, navigator tools and longer evidence remain available on demand; do not restore the old stacked command band or permanent analysis-queue panel by treating earlier layout notes as blockers.
- Smaller scoped Binary View typography and the mockup's pane proportions are intentional; shell execution-button font/metrics and zoom/DPI application remain independent. Real FILE/LIVE ownership, input/navigation semantics and register/branch validation still apply. See `docs/MOCKUP_FIDELITY_V2.md` for this revision's verification.
- This revision passed Release x64, widget/workbench/feature tests, startup smoke and the full production-object regression suite with both live scrolling and live patch restoration enabled. The final suite has zero failures; native screenshot review covers compact/desktop, Light/Midnight and preserved execution controls. Test clicks must use actual logical column bounds because the flow gutter now sits beside instructions, and Bytes is hidden by default.
...
## September 2026 — offline Peggle cursor experiment

- Added the standalone native `tools/peggle_cursor` helper, build wrapper and read-only ProcessMemorySession/Zydis probe; this is an offline game experiment, not a DisasmStudio scripting/plugin API. Exact running-build signatures, process creation identity, object types, bounded scene lists and paused-thread checks guard temporary capture patches.
- Verified native cursor capture and release in English Steam Peggle Deluxe 1.01: the game owns pointer/reference changes, original code is restored after capture, and native release restores normal flight and original mouse settings. Release before menus/replay/quit; scene ownership changes must never cause stale-pointer writes.
- Corrected the requested behavior from stationary native dragging to an active captured ball so ordinary peg collisions remain eligible. Keep the exact captured ball identity, clear only its stationary byte while following, and restore the native held state before click-driven release. Collision responses and large cursor jumps can affect positional continuity; no global collision bypass is used.
- Added bounded read-only peg hit-flag observations to verify actual collision effects. See `tools/peggle_cursor/README.md` for native address provenance, controls and verification limits.
- Verified collision-enabled cursor control in a restarted game with 58 exact object/PegInfo false-to-true hit transitions; observed native release after ball-list removal and a subsequent active capture with original instruction bytes restored. The final `/W4` build is staged in `build/peggle_cursor_updated` while the earlier collision-enabled helper remains open.
...
## September 2026 — live browsing and string action trails

- Live Assembly permits repeated earlier/later browsing through contiguous readable memory. Preserve committed instruction checkpoints through ambiguous x86 prefix bytes and content refreshes; continuously held scrollbar and arrow-key input must keep refilling the bounded window without changing selection/history. Real unreadable boundaries retain the useful current rows. See docs/LIVE_ASSEMBLY_SCROLLING.md and the contained long-distance live_scroll_fixture.inc.
- Strings now expose Trace related value changes through Core/StringActionTrace and Tabs/BinaryViewStringTrace. The shared analysis worker follows verified references, owned CFGs and bounded direct-call neighborhoods to typed arithmetic/stores, preserving producer/write addresses, candidate naming and explicit scope. LIVE starts require exact FILE-image mapping; result actions keep FILE provenance and recheck runtime mapping. Retire on image/analysis/index/session changes and do not let Retry restamp old string evidence into a replacement owner.
- FunctionNamer only adds message/action candidate names after stronger evidence. Direct stack/frame mechanics, unknown/clobbered register lineage, conflicting/negated wording, mutually exclusive paths and known noreturn continuations cannot support the new claims. Signed negative immediate arithmetic is classified by effect direction. A string such as added! cannot alone identify a points field. See docs/STRING_ACTION_TRACING.md.
- LiveScanService distinguishes successful xref jobs from complete search coverage: preserve partial hits, report capped bytes/ranges, short/unreadable reads and incomplete region inventories, and retain actual decoder errors. Never report zero hits as exhaustive when coverage is incomplete.
- Verified Release x64 and the full production-object UI suite with the contained live scrolling case: zero failures, 69,888 bytes of held-scrollbar upward travel and 69,892 downward, with all 129 production objects matching the final build. Logs: build/live-review-final-scroll-build.log, build/live-scroll-focused.log and build/live-review-final-full-suite.log. The focused string action trace, namer, namer pipeline, LIVE scan and analysis service tests also passed. ImGui table scopes change GetWindowScrollbarID's current seed; held input must use the native child-window seed.

## September 2026 — function-name accuracy and backtrace

- FunctionNamer uses exact decoration-normalized API families and bounded entry-reachable evidence clipped at the next known function. Partial bodies cannot support narrow complete-operation claims. Preserve authoritative flow, delay slots, accumulator width/clobber guards, analyst/export names, cancellation and deterministic deduplication; analysis schema is 7.
- Call Stack is reachable through Debug/More/Ctrl+K Backtrace. Frame zero is the captured current context; older rows retain return continuations, pc-1 caller attribution and separately labelled possible CALL boundaries. Scanned stack values are explicit candidates, never an unwound caller chain.
- Refresh runs on the existing held debug-event worker. Exact session/thread/revision/module ownership, register/memory/module retirement and generation-checked publication prevent stale backtraces; browsing never executes or rewinds the target. Keep DbgHelp off the renderer and preserve FILE/LIVE navigation.
- See docs/FUNCTION_NAMES_AND_BACKTRACE.md, backtrace_test, backtrace_ui_fixture.inc and the contained self-child DS_X64_BACKTRACE_ONLY regression.
- Main-agent final verification: Release x64/helper build, six focused Core tests, native seven-frame backtrace test, 16,384-function naming benchmark, startup smoke and full production UI regressions passed with zero final failures. All 129 UI-test objects match the final build; Light/Midnight compact/desktop captures reviewed. Logs: build/naming-backtrace-review/.
