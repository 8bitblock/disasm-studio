...
src/
  ...
  Tabs/
    ...
    BinaryViewTab          Assembly + Pseudocode + Hex, side panel (Bookmarks/Functions/
                           Strings/Exports), byte-pattern search, lower tabs (Breakpoints/Notes/
                           Results/Hotkeys)
...
- The Binary View's filterable **PE Exports** side panel shows named aliases, ordinal-only entries, forwarded exports, and local code/data targets; local targets navigate to code or Hex.
- **File > Open Binary** loads an `.exe/.dll/.sys/.bin` (PE/ELF/Mach-O auto-detected); **Open as Raw…** maps a flat blob at a chosen base + arch as one complete executable `.raw` section. The selected base is an explicit analysis/function root (including VA 0), while the selected architecture controls architecture-specific discovery. Raw files participate in the normal background analysis, full-program listing, xrefs, and call graph; a mapping whose final VA would overflow is rejected. **Save Binary As…** writes a patched copy. The Projects tab lists recent targets (double-click to reopen) and shows parsed format, image base, entry point, sections, and the saved-analysis summary.
...
- **Automatic function discovery** (`FunctionAnalyzer`): entry point or explicit raw-base root + PE export table seeds, recursive-descent call following, and an architecture-gated prologue heuristic sweep.
...
- **Multi-format loader**: PE32/PE32+, **ELF (32/64)**, **Mach-O (thin 32/64)** header parsing, plus **File ▸ Open as Raw…** to map a flat code blob (shellcode/firmware) at a chosen base + arch. Raw loads get a complete executable `.raw` section and an explicit function root at the selected base without fabricating a header entry point. The CPU is read from structured headers (`machine()`); for raw files, the analyst's exact architecture selection drives decoding and architecture-specific discovery.
