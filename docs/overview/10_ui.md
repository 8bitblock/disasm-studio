...
- **Fixed browser-style shell.** The workbench's core panel arrangement is app-owned rather than a user dockspace. A top document strip switches among at most eight retained static/live-image contexts, followed by the global debug toolbar and the section-card strip; topology changes are queued until the frame releases all document references. ImGui platform viewports remain enabled for platform-window/DPI bookkeeping, and `DpiEnableScaleViewports` preserves logical geometry across monitor changes.
...
- **Bounded user preferences.** The **View** submenus update the active palette/density, while
  Symbol Settings stores an opt-in network policy/cache and Investigation stores at most 32
  encoded recent queries in `%APPDATA%\DisasmStudio\prefs.ini`. `loadPrefs()` restores the
  visual values before `ApplyTheme`; symbol fetching remains off until explicitly enabled.
...
- **Preferences are not analysis.** The prefs file holds UI, symbol-policy, and recent-query
  settings; authoritative per-binary analysis remains in atomic project sidecars and ImGui keeps
  its own layout file.
